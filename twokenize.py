...
r"va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|za|zm|zw)"	# bTODO: remove obscure country domains?
...
# Numeric
timeLike = r"\d+(?::\d+){1,2}"
# print("timeLike: ", timeLike)
#numNum     = r"\d+\.\d+"
numberWithCommas = r"(?:(?<!\d)\d{1,3},)+?\d{3}" + r"(?=(?:[^,\d]|$))"
# print("numberWithCommas: ", numberWithCommas)
...
bfCenter = r"(?:[\.]|[_-]+)"
bfRight = r"\2"
# print("shujude leixing:\n", type(bfLeft), type(bfCenter), type(bfRight))
s3 = r"(?:--['\"])"
s4 = r"(?:<|&lt;|>|&gt;)[\._-]+(?:<|&lt;|>|&gt;)"
s5 = "(?:[.][_]+[.])"
# myleott: in Python the (?i) flag affects the whole expression
# basicface = "(?:(?i)" +bfLeft+bfCenter+bfRight+ ")|" +s3+ "|" +s4+ "|" + s5
basicface = "(?:" + bytes.decode(bfLeft) + bfCenter + bfRight + ")|" +s3 + "|" + s4 + "|" + s5
# print(basicface)

eeLeft = r"[＼\\ƪԄ\(（<>;ヽ\-=~\*]+"
eeRight = u"[\\-=\\);'\u0022<>ʃ）/／ノﾉ丿╯σっµ~\\*]+".encode('utf-8')
eeSymbol = r"[^A-Za-z0-9\s\(\)\*:=-]"
eastEmote = eeLeft + "(?:"+basicface+"|" + eeSymbol+")+" + bytes.decode(eeRight)
...
Arrows = regex_or(r"(?:<*[-―—=]*>+|<+[-―—=]*>*)", bytes.decode(u"[\u2190-\u21ff]+".encode('utf-8')))
